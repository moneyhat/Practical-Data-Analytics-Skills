...
#making a categorical feature from numerical data
#we would want to know how much wear and tear by manupulating the variables to provide additional information
...
#mph = distance(miles)/ end time-start time [possible through lubridate]

#make mi/hr feature
#dist -> dist_miles
#time -> convert dt end_time -start_time
#divide dist/time

#rider_speed <-  (bikes$distance_miles)/ (bikes$end_time - bikes$start_time)
...
head(bikes$start_time)
tail(bikes$start_time)
xxx <- ymd_hms(bikes$start_time)
glimpse(xxx)
...
bikes <- bikes %>%
  mutate(trip_time = bikes$end_time - bikes$start_time)

#Note:
# Mutate is column name you want to create = whatever math you want to do on the column
...
summary(as.numeric(bikes$trip_time))
...
#run this again to see the difference (we only want to see people who are on the bike for a certain amount of time)
ggplot(bikes,
       aes(x = trip_time)) +
  geom_histogram()
...
#convert to numeric, then divide
...
#       tourists ride longer and ride slower
#           locals ride faster and quicker
...
#goal: create route feature

#format: from_id - ti_id 
...
bikes <- bikes %>%
  unite(route,
         c('from_station_id','to_station_id'), sep = '-', remove = FALSE)

glimpse(bikes)


# now let's quick convert route to a factor so we get a summary of how many are present. Convert the style column to a factor using factor()

bikes$route <- factor(bikes$route)
summary(bikes$route)

#we can now see the to and from routes people are taking 
