...
#creates a year column extraxt digits 0-9 and make it 4 in a row
...
# Note: Student IDs are numerical, but you can't add them (nominal)
...
# converting to numerics is pretty straightforward.  It's also nice as now we can graph using year on an an axis and it will be in order. if you have a list of 100 numbers, you might come across NA Values, it automatically converts values to NA which is nice
...
#Note: the order on the x axis is not intuitive (out of order!!)
...
beer$popularity <- factor(beer$number_of_reviews, levels = c('not', 'moderate', 'very'))
...
xxx <- dmy(beer$add_date)
# now that it worked, make the new column

#Note:  for 744 of these, R Studio can't figure it out and doesn't know what to do with them and you need to check your year date format
...
#Note: not a great representation for validation.  Adjust it by figuring out how long the beer has been out. converting to rates/percentages is probably more useful
...
#Note: R is counting how many days its been since 1970 we're getting closer to making our rate. we can delete the "7" and appropriate the number of days, because the weeks are currently being displayed
...
#can't divide by a difftime object, so we need to divide by a numeric value
